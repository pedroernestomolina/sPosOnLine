...
{
    public class Pago : PosOnLine.Src.Pago.IPago
    {
        ...
        public decimal MontoDivisa
        {
            ...
            {
                var x = 0.0m;
                x = _detalle.Where(w => w.Modo ==  Src.Pago.Procesar.Enumerados.ModoPago.Divisa).Sum(s => s.Monto);
                return x;
            }
        }
        ...
        {
            var rt = "";
            var det = _detalle.FirstOrDefault(f => f.Modo == Src.Pago.Procesar.Enumerados.ModoPago.Electronico && f.Id == id);
            if (det != null) 
            {
                rt = det.Lote;
            }
        ...
        {
            var it = _detalle.FirstOrDefault(f => f.Modo == Src.Pago.Procesar.Enumerados.ModoPago.Efectivo);
            if (it == null)
            {
                if (monto > 0m)
                {
                    it = new PagoDetalle()
                    {
                        Modo = Src.Pago.Procesar.Enumerados.ModoPago.Efectivo,
                        Tasa = 1,
                        Cantidad = 1,
                        Monto = monto,
                        Lote = "",
        ...
        protected bool _activarBonoPorPagoDivisa;
        ...
        public void AddDivisa(decimal monto)
        {
            ...
            var it = _detalle.FirstOrDefault(f => f.Modo == Src.Pago.Procesar.Enumerados.ModoPago.Divisa);
            var it2 = _detalle.FirstOrDefault(f => f.Modo == Src.Pago.Procesar.Enumerados.ModoPago.Electronico && f.Id==4);
            if (it != null)
            {
                _detalle.Remove(it);
            }
            ...
            if (monto > 0m)
            {
                ...
                it = new PagoDetalle()
                {
                    Modo = Src.Pago.Procesar.Enumerados.ModoPago.Divisa,
                    Tasa = _tasaCambio,
                    Cantidad = monto,
        ...
        public void AddElectronico(decimal monto, int id, string nroLote="", string nroReferencia="", bool activar=true)
        {
            ...
            var it = _detalle.FirstOrDefault(f => f.Modo == Src.Pago.Procesar.Enumerados.ModoPago.Electronico && f.Id == id);
            if (it != null)
            {
                _nroLote = it.Lote;
            ...
            if (monto > 0m)
            {
                ...
                {
                    Id = id,
                    Modo = Src.Pago.Procesar.Enumerados.ModoPago.Electronico,
                    Tasa = 1,
                    Cantidad = 1,
                    Monto = monto,
                    Lote = nroLote,
        ...
        { 
            ...
            { 
                var _monto=0m;
                var _ent = _detalle.FirstOrDefault(f => f.Modo == Src.Pago.Procesar.Enumerados.ModoPago.Electronico && f.Id == 4);
